...
void EllipseItem::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget *widget)
{
    ...
    if (isSelected() ){ // draw dashed line ellipse
        painter->setPen( Qt::DashLine );
        painter->drawEllipse(rect());
    }
    else
        painter->drawEllipse(rect());// Draw normal ellipse
}
...
void EllipseItem::resizeLeft(const QPointF &pt)
{
    QRectF tmpRect = rect();
...
void EllipseItem::resizeRight(const QPointF &pt)
{
    QRectF tmpRect = rect();
...
void EllipseItem::resizeTop(const QPointF &pt)
{
    QRectF tmpRect = rect();
...
void EllipseItem::resizeBottom(const QPointF &pt)
{
    QRectF tmpRect = rect();
    if( pt.y() < tmpRect.top() )
        return;
